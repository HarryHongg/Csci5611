...
void setup(){
  ...
  agentPos[3] = new Vec2(440,210);
  agentPos[4] = new Vec2(250, 100);
  goalPos[0] = new Vec2(200,420);
  goalPos[1] = new Vec2(120,120);
  goalPos[2] = new Vec2(220,220);
  goalPos[3] = new Vec2(280,100);
  goalPos[4] = new Vec2(600,300);
...
Vec2 computeAgentForces(int id){
  ...
  for(int i=0; i<numAgents; i++){
    if(id != i){
      ...
      if(ttc > 0){
        Vec2 future_A = agentPos[id].plus(agentVel[id].times(ttc));
        Vec2 future_B = agentPos[i].plus(agentVel[i].times(ttc));
