...
float obstacleSpeedList[] = {obstacleSpeed, obstacleSpeed*2};
float COR = 0.7;
...
void update(float dt){
  ...
  if (shiftPressed) {
    obstacleSpeed = obstacleSpeedList[1];
  }
  else {
    obstacleSpeed = obstacleSpeedList[0];
  }
  ...
  if (obstacleVel.x != 0  && obstacleVel.y != 0){
    obstacleVel.normalize();
    obstacleVel.mul(obstacleSpeed);
  }
  ...
  for (int i = 0; i <  numParticles; i++){
    ...
    if (pos[i].distanceTo(spherePos) < (sphereRadius+r)){
      ...
      Vec2 obstacleVelNormal = normal.times(dot(obstacleVel,normal));
      velNormal.add(obstacleVelNormal);
